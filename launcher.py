...
class launcher:
    ...
    def monitor_subprocess(self, process, browser, mode, scheduled_window, logdir, logmode):
        #<Sumamry> 
        ...
        while(1):
            ...
            if process.returncode != 0 and count < 4: 
                self.logger.error(f'Subprocess: {browser}_reader.py exited with error')
                self.logger.debug(f'Retrieved code {process.returncode}')
                self.logger.info(f"Relaunching the subprocess: {browser}_reader.py")
                process = subprocess.Popen(['python', f'{browser}_reader.py', mode, scheduled_window, logdir, logmode], stderr=subprocess.PIPE)
                self.logger.info("Relaunched the reader that exited with error")
                count += 1
            elif count > 3:
                ...
                self.logger.info("Subprocess completed with no errors; Monitor thread is exiting")
                break
    
    def launch_reader(self, browser, mode, scheduled_window, logdir, logmode):
        #<Summary>
        # launch_reader function:
        #   launches the reader based on the browser provided
        #</Summary> 
        ...
        monitorThread = threading.Thread(target=self.monitor_subprocess, args=(process, browser, mode, scheduled_window, logdir, logmode,))
        monitorThread.start()
    ...
    def start(self):
        ...
        if self.options['browser'] == 'all':
            for browser in self.installed_browsers:
                self.launch_reader(browser, self.options['mode'], self.options['scheduled_window'], self.options['logdir'], self.options['logmode'])
        ...
        if self.options['browser'] in self.installed_browsers:
            ...
            if all(reader in allowed_browsers for reader in readers_list):
                readers_to_launch = [reader for reader in readers_list if reader in self.installed_browsers]
